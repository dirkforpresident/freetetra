...
function stationIcon(stationType: Station["type"], online: boolean): L.DivIcon {
  ...
  const symbol = stationType === "hotspot" ? "H" : stationType === "tmo_site" ? "T" : "B";
  const html =
...
function stationPopup(st: Station): string {
  const typeName =
    st.type === "hotspot" ? "Hotspot" : st.type === "tmo_site" ? "TMO-Site" : "BlueStation";
  const dot = st.online
...
async function loadHexes() {
  ...
  try {
    ...
    if (s) {
      ...
      statTMOSites.value = online + "/" + total;
    ...
    <div class="header">
      ...
      <div class="info">
        <span class="stat-group"
          ...
          <b>{{ statTMOSites }}</b> {{ t("map.tmo_site") }}</span
        >
